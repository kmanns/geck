...
- `_video.json`: Defines the DA/Universal Editor block field, repeatable video items, and the `videoUrl` input field

## da.live Authoring

### Supported Field

- `videoUrl`: Paste one full video URL into each Video block row

### DA Usage

In `da.live`, add a Video block from the section insert menu and populate the initial `Video URL` field with a URL such as:
...
- The first video URL is authored on the Video block itself.
- Additional video URLs are authored as repeatable Video items in Universal Editor.
- Use the `+` button on the Video block to add another Video item row when UE exposes the repeatable control.
